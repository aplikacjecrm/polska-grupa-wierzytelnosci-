...
if (window.ChatManager) {
    ...
    setupSocketListeners() {
        ...
        socketManager.on('new-chat-message', (message) => {
            console.log('📨 Otrzymano wiadomość:', message.message, 'ID:', message.id);
            
            // DEDUPLIKACJA: Sprawdź czy już mamy tę wiadomość
            if (this.messages.find(m => m.id === message.id)) {
                console.warn('⚠️ DUPLIKAT! Wiadomość ID', message.id, 'już istnieje - pomijam');
                return;
            }
            
            // Jeśli wiadomość jest od aktualnie wybranego użytkownika
            if (this.currentUser && message.sender_id === this.currentUser.id) {
    ...
    async selectUser(userId) {
        ...
        this._refreshInterval = setInterval(async () => {
            if (this.currentUser && this.currentUser.id === userId) {
                console.log('🔄 Auto-odświeżanie wiadomości...');
                const currentScrollPos = document.getElementById('messagesList')?.scrollTop;
                ...
                if (!wasAtBottom && currentScrollPos !== undefined) {
                    document.getElementById('messagesList').scrollTop = currentScrollPos;
                }
            }
        }, 30000); // 30 sekund
        
        console.log('✅ Auto-odświeżanie włączone (30s)');
    }
