...
if (window.ChatManager) {
    ...
    setupSocketListeners() {
        if (!window.socketManager || !window.socketManager.connected) {
            console.warn('⚠️ [BIG CHAT] SocketManager nie jest połączony - retry za 500ms...');
            console.log('   - socketManager exists:', !!window.socketManager);
            console.log('   - socketManager.connected:', window.socketManager?.connected);
            setTimeout(() => this.setupSocketListeners(), 500);
            return;
        }
        
        // ❌ NIE CZYŚĆ WSZYSTKICH LISTENERÓW! To usuwa floating chat i powiadomienia!
        // Zamiast tego - stary listener dużego czatu automatycznie zostanie zastąpiony
        console.log('🔌 [BIG CHAT] Dodaję socket listenery...');
        console.log('📊 [BIG CHAT] Aktualna liczba listenerów new-chat-message:', 
            socketManager.listeners['new-chat-message']?.length || 0);
